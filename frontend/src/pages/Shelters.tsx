...
import type { Shelter, Shade } from '../types/api'
...
// C-04 / SC-08 실내 대기 장소(카카오) + 야외 그늘막(전국그늘막쉼터) — 도보 가까운 순
export default function Shelters() {
  ...
  useEffect(() => {
    // 현재 위치 기준으로 실내 대기 장소 + 야외 그늘막 동시 조회
    const load = (lat: number, lng: number) => {
      api.getShelters(lat, lng).then(setList).catch(() => setError(true))
      api.getShades(lat, lng, 800).then(setShades).catch(() => setShades([]))
    }
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        (p) => load(p.coords.latitude, p.coords.longitude),
        () => load(FALLBACK.lat, FALLBACK.lng), // 거부·실패 시에만 폴백
        { timeout: 4000 },
      )
    } else {
  ...
  return (
    <div className="page shelter-page">
      ...
      {shades && shades.length > 0 && (
        <>
          <p className="muted" style={{ marginTop: 16 }}>🌂 야외 그늘막 · 잠깐 햇빛 피하기</p>
          {shades.map((s, i) => (
            <a key={`${s.lat},${s.lng},${i}`} className="card shelter-item" href={kakaoLink(s.name, s.lat, s.lng)} target="_blank" rel="noreferrer">
              <span className="shelter-icon">🌂</span>
              <div className="shelter-info">
                <strong>{s.name}</strong>
                <span className="muted">{s.address || s.type || '야외 그늘막'}</span>
              </div>
              <div className="shelter-dist">
                <strong>도보 {s.walk_minutes}분</strong>
                <span className="muted">{s.distance_m}m</span>
