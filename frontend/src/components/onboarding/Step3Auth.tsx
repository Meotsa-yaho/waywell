    ...
    >
      ...
      <div className="space-y-1.5">
        ...
        <h1 className={`text-xl sm:text-2xl font-bold leading-tight tracking-tight text-emerald-900 dark:text-white`}>
          노출 부하 프로필이<br />
          <span className="text-emerald-700 dark:text-emerald-400">완성되었습니다!</span>
        </h1>
      ...
      <div className="my-4 space-y-3">
        ...
        <div
          ...
          className={`rounded-3xl p-5 text-center border shadow-xs transition-colors bg-emerald-50 border-emerald-100/90 dark:bg-emerald-950/30 dark:border-emerald-900/60`}
        >
          ...
          <div className={`text-sm sm:text-base font-bold text-emerald-950 dark:text-white`}>
            {currentOption.tag} 적용됨
          </div>
          <p className={`text-[11px] mt-1 font-medium text-emerald-800/80 dark:text-emerald-300`}>
            "{currentOption.title}"
          </p>
        </div>
        ...
        <div
          ...
          className={`flex items-center justify-between p-3 border rounded-2xl cursor-pointer transition-colors bg-slate-50 hover:bg-slate-100/70 border-slate-100 dark:bg-slate-900/80 dark:hover:bg-slate-800/80 dark:border-slate-800`}
        >
          <div className="flex items-center gap-2.5">
            <div className={`w-7 h-7 rounded-lg flex items-center justify-center bg-emerald-100 text-emerald-700 dark:bg-emerald-950 dark:text-emerald-400`}>
          ...
          <button
            ...
            className={`w-full py-3 border rounded-xl text-xs font-bold transition-all cursor-pointer flex items-center justify-center gap-1 focus:outline-none focus:ring-2 ${
              isKakaoLoading
                ...
                : 'border-slate-200 hover:bg-slate-50 text-slate-600 focus:ring-slate-300 dark:border-slate-800 dark:hover:bg-slate-800 dark:text-slate-300 dark:focus:ring-slate-700'
            }`}
          >
            {authSuccess === 'guest' ? (
              <>
