...
export function LocationConsent({ isDarkMode = false, onAllow }: { isDarkMode?: boolean; onAllow: () => void }) {
      ...
      {show && (
        ...
        >
          ...
          >
            ...
            <p className={`text-xs leading-relaxed mb-2 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`}>
              현재 위치의 날씨·미세먼지와 출발지를 자동으로 맞춰드려요.
            </p>
            <div className={`flex items-start gap-1.5 text-[11px] mb-4 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>
